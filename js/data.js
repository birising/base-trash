...
// Přímý přístup blokuje CORS, proto použijeme veřejné proxy, ale stále čteme přímo S3 data.
const streamCsvSources = [
  { url: `https://corsproxy.io/?${encodeURIComponent(streamCsvUrl)}`, label: "corsproxy.io" },
  { url: `https://api.allorigins.win/raw?url=${encodeURIComponent(streamCsvUrl)}`, label: "allorigins" },
];
...
async function fetchStreamCsvWithProxy() {
  const sources = streamCsvSources;
  let lastError = null;
