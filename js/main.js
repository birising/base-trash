window.addEventListener("DOMContentLoaded", async () => {
  ...
  async function fetchStreamLevel() {
    try {
      const response = await fetch(streamCsvUrl, { cache: "no-store" });
      if (!response.ok) throw new Error(`HTTP ${response.status}`);
      const csv = await response.text();
      const parsed = parseStreamCsv(csv);
      if (parsed.length) {
        setStreamHistory(parsed);
        streamState.status = `Data z water_level.csv · ${new URL(streamCsvUrl).hostname}`;
      } else {
        throw new Error("Prázdný soubor water_level.csv");
      }
    } catch (err) {
